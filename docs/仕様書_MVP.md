...
2026-08-31 初版 ／ 2026-09-02 改訂（画面①実装・UI刷新・用語統一〔面接→ミーティング／候補者→ゲスト／面接官→参加者／採用担当者→主催者〕を反映） ／ 2026-09-03 改訂（§6.2 ログインをパスワード認証に、§6.1 に `admin` を追加、§6.3〜§6.5 の画面②③を実装済みに、§7 ルーティングを実装状況に合わせ更新、§9.4 の未着手Must を縮小、§10 の F-25/F-26 を実装済みに。詳細は判断メモ D-1〜D-7） ／ Ruby 3.2+ ／ Rails 8.1 ／ SQLite3 ／ Hotwire ／ Minitest
関連文書：要件定義書（Phase別）、別紙A（実装詳細）、判断メモ
...
| 開始時刻 | `<input type="time" step="300">`（5分刻み） | 予約フォーム（①b）。週カレンダーでは**緑の空き帯そのものをクリック**し、押した縦位置（時刻軸に対応）を5分丸めした時刻が①bの開始時刻の初期値になる。クリック後、①bの開始時刻欄で任意の5分刻みへ変更できる |

**空き帯クリックは入力補助であり、制約ではない。** 週カレンダーの空き帯は内部的に5分刻みで走査しており（§6.6）、帯のどこをクリックしても「その時刻から所要時間ぶんが全員空き」であることは走査で保証済み（帯の末尾＝所要が収まる最後の開始でクランプ）。**任意の5分刻みで開始したい場合は、帯をクリックして予約フォームを開いたのち開始時刻欄を直接書き換える。** 縦位置→時刻の変換は Stimulus（`slot_picker_controller.js`）が担い、JS無効時やキーボード操作では帯の先頭時刻にフォールバックする。
...
|---|---|---|---|
| **メンバー**（社内の人） | ○ | ○ | `users` に行があること |
| **管理者** | ○（設定画面も） | ○ | `users.admin = true` |
| **ゲスト** | × | × | `meetings.guest_name` |

**主催者が自分でミーティングに出るケースがあるため、操作する人と出る人を別テーブルにしない。** 分けると兼任者が2レコードに分かれ、氏名の二重管理が発生する。

**当初は `operator`（ツールを操作する）と `participant`（ミーティングに出る）の2フラグで兼任を表現していたが、実運用では両方 true のアカウントしか作られず、区別が設定項目としてだけ残っていた。判断メモ D-15 で両フラグを廃止し、「アカウントを持つ人＝メンバー（ログインでき、参加者候補にも並ぶ）」に一本化した。**

| | `admin` | できること |
|---|---|---|
| メンバー | false | ログイン、ミーティングを組む・一覧・変更・キャンセル、参加者候補に並ぶ |
| 管理者 | **true** | 上記に加えて設定（メンバー管理・テナント設定）を操作できる |

`admin` は設定画面の操作可否だけを分ける1段階のフラグ。「管理者」と「CA」で操作をどう分けるかは未ヒヤリング（仮定A-6）で、細分化と管理者画面の完全分離は Phase 2（判断メモ D-2）。

### 6.2 ログインの実装（メール＋パスワード認証）

**当初は「`operator` を選択してセッションに保持するだけ」の簡易ログインだったが、メンバー管理（画面③）と認可（`admin`）を Phase 1 で実装するにあたり、`has_secure_password` による認証へ置き換えた。経緯は判断メモ D-1。**

> **以下のコードは `has_secure_password` 時代のもので、現在の実装ではない。** その後 Devise へ移行し（判断メモ D-12）、`operator` フラグ自体も廃止した（判断メモ D-15）。判断の経緯を残すためこのまま置いている。現在の姿はこの節の末尾にまとめている。
...
class SessionsController < ApplicationController
  ...
  def new; end # GET /login（ログイン済みなら確認表示のみ）
...
class ApplicationController < ActionController::Base
  ...
  # 設定画面（メンバー管理）だけに許す操作で使う（UsersController）
  def require_admin
...
| 項目 | 内容 |
|---|---|
| 認証 | メールアドレス＋パスワード（Devise `database_authenticatable`。判断メモ D-12） |
| 保持先 | Rails のセッション（Cookie に署名付き） |
| 固定化対策 | ログイン成功時に `reset_session` してから `user_id` を格納 |
| ログアウト | `DELETE /logout` → `reset_session` |
| 切り替え | ログアウトして別アカウントでログイン |
| 未ログイン時 | `before_action` で全画面から `/login` へリダイレクト |

**この節のコード例は初版（`has_secure_password` 時代）のもので、現在の実装は Devise に置き換わっている（判断メモ D-12）。** また、当初あった「`operator` に絞ってログインさせ、毎リクエスト確認する」仕組みは、`operator` フラグの廃止（判断メモ D-15）とともに削除した。**アカウントを持つ人は全員ログインできる。** アクセスを止める手段はメンバーの削除（管理者のメンバー管理）に一本化されている。

**据え置き（Phase 2 以降）**：多要素認証・パスワードリセット・ロックアウト・監査ログ。**Phase 1 で認証・認可の土台は入ったが、F-52 として残すのはこれらと管理者画面の完全分離。**
...
**権限による出し分けは設定メニュー（画面③メンバー管理）のみ。** `admin = true` のときだけ設定リンクを表示する。それ以外の画面はメンバー間で区別しない（管理者とCAの操作差は未ヒヤリング＝仮定A-6）。右上に現在の操作者と「ログアウト」を常時表示する。
...
週カレンダーは1週間×7日を、選択メンバーの予定を重ねた1つのカレンダーとして表示する（§6.6）。左端に時刻軸（正時ラベル＋薄い横罫）と「終日」行を置き、縦位置を読み取れるようにしている。凡例は4種類。

| 表示 | 意味 |
|---|---|
| 緑（空き） | 選択メンバー**全員**がその区間に空いている |
| グレー（予定あり） | 誰か1名以上の実際の予定。氏名と件名をラベル表示。**終日予定は本体ではなく最上部の「終日」行に出す** |
| 斜線・赤（予約不可） | `block` ルール（昼休みなど）に該当 |
| 斜線・グレー（営業日外） | その曜日に `allow` ルールが無い |

緑の空き帯をクリックすると、下に予約フォーム（①b）が開く。押した縦位置（時刻軸に対応）を5分丸めした時刻が開始時刻の初期値になり、開始時刻・所要時間はここで自由に微調整できる（§4.2）。ゲスト名・実施方法・場所・会議URLを入力して「予約する」を押すと確定直前に再判定される（§5）。

![予約フォーム](images/screen1b_booking_form.png)

**「あと1人で空き」（オレンジ枠）は追加後に撤回した**：全員空きの枠に加えて「ちょうど1名だけ埋まっている」枠もオレンジで出す機能を実装したが、ニアミス枠が本人の「予定あり」ブロックと必ず同じ時間帯に重なり、人名が二重表示になるため削除した（`WeeklySlotFinder#near_windows`／別紙A 第5版）。誰の予定かは通常の「予定あり」ブロックの表示で足りる。

**時刻軸・「終日」行の追加（実装段階のUI追加）**：週カレンダー左端に、表示レンジ内の正時ラベルと薄い横罫を並べた時刻軸列を置いた。あわせて、それまで本体内に `position:static` で流し込んでいた終日予定を、7日分そろえた固定高の「終日」行に分離した（時間指定の予定と視覚的に混ざらず、本体の開始位置も全列でそろう）。判定ロジック・データモデルには影響しない表示のみの変更。
...
`MeetingsController#index` ＋ `Meeting.upcoming` スコープ。**登録者で絞り込まず全件共有／今後の予約のみ表示**（§6.7・§9.1-2）。各行から詳細（`show`）・変更（`edit`）・キャンセル（`destroy`）へ遷移する。過去分を参照する画面は作っていない（H-36 未回答）。判断メモ D-3。上図は設計時のモックアップで、旧用語のまま。

#### メンバー管理（画面③）── ◎実装済み（admin 限定）

![設定メニュー（モックアップ）](images/screen3_nav_settings.png)

`resources :users` のフルCRUD。`before_action :require_admin` で `admin = true` のメンバーだけが操作できる（§6.2）。氏名・メール・パスワード・`admin` を編集する（判断メモ D-15 で `operator`/`participant` は廃止）。
**削除の制約**：登録者になっているミーティングが残っているメンバーは削除できない（`User has_many :created_meetings, dependent: :restrict_with_exception`）。自分自身も削除不可。編集時にパスワード欄が空なら変更しない。判断メモ D-2・D-4。上図は設計時のモックアップで、旧用語のまま。

#### テナント設定（画面⑤／`/tenant_rules`）── ■実装済み

F-11〜F-13（営業時間・予約不可時間の設定）。テナント全体（`user_id: nil`）の `allow`（営業時間）と `block`（固定ブロック）を、設定タブ配下の**1画面に2セクションで**並べて CRUD する（admin限定。判断メモ D-11・D-13）。この2つは BR-12 で合成されて初めて「予約できる時間帯」が決まるため分離しない。

営業時間の表は日〜土の**7曜日すべて**を出し、`allow` ルールが1件も無い曜日は「営業日外」と表示する（§4.1 の `business_hours_reason` は allow 不在の曜日を終日不可として扱うため、一覧から消すと設定者から見えなくなる）。ラベル欄は `block` にのみ表示する（`label` は不可理由「予約不可時間（昼休み）」として使われ、allow 側は判定でも表示でも参照しないため）。

**個人ルール（`user_id` あり）の設定画面は未実装。** この画面は `AvailabilityRule.tenant_wide` に絞って find するため、URLのidを個人ルールに差し替えても404になる。

#### カレンダー連携（画面④）── ▢未実装

F-29（カレンダー連携画面）は未実装。この画面が無くても §6.6 の主動線（ミーティングを組む）は動作する。§9.4。

### 6.5 各画面の入出力

| # | 画面 | 誰が | 入力 | 出力 | 優先度 |
|---|---|---|---|---|---|
| ⓪ | ログイン | 主催者 | メールアドレス＋パスワード（メンバーは全員ログインできる） | セッションに `user_id` を保持 | Must（◎実装済み） |
...
| ② | 予約一覧 | 主催者 | （なし） | 今後のミーティングを日時昇順で表示。各行から詳細・変更・キャンセルへ | Must（◎実装済み） |
| ②b | 予約の変更 | 主催者 | 日時／所要時間／**参加者**／ゲスト名／実施方法／場所／会議URL | 再判定のうえ変更確定 | 実装済み（F-25 を Phase 2 から前倒し。判断メモ D-5。参加者の選び直しは D-14 で追加） |
| ③ | メンバー | **管理者** | 氏名／メール／パスワード／`admin` | メンバー一覧 | Must（◎実装済み・admin限定） |
| ④ | カレンダー連携 | 主催者 | — | ダミーカレンダーの接続状態。**Phase 2でOAuth接続に置き換わる** | 実装中に追加（▢未実装） |
| ⑤ | 営業時間 | 管理者 | 曜日／時間帯／allow・block／ラベル | ルール一覧 | Should（▢未実装） |
...
| 5 | 緑の空き帯をクリック | 日時・参加者が入った予約フォームを開く。**押した縦位置＝開始時刻（5分丸め）が初期値**。開始時刻・所要時間はここで自由に微調整できる（§4.2） |
...
| 状態 | 表示 | 補足 |
|---|---|---|
| 全員空き | **クリックできる緑の帯**（`a.seg--free`） | 帯は「空き」＋時間幅（`10:00〜12:00`）を表示。クリック位置の時刻を5分丸めして①bへ渡す |
| 誰かに予定あり（時間指定） | 灰色のブロック | **誰の何の予定かをラベル表示**（例：伊藤／部門定例）。複数人の予定が同じ時間帯に重なる場合は列を分けて横に並べる |
| 誰かに終日予定 | 最上部の「終日」行に灰色のブロック | 本体には描かない。氏名と件名をラベル表示 |
| 営業時間外・営業日外 | 背景をグレーアウト | 枠を描かない |
| 予約不可時間 | 斜線パターン | ラベルを表示（例：昼休み） |
| 時刻軸 | 左端の列 | 表示レンジ内の正時ラベルと、本体に薄い横罫（60px＝1時間） |
...
**「あと1人で空き」（ニアミス枠）は実装したのち撤回した**：走査ループ内で不可の人数がちょうど1名の区間を `near_windows` として別途拾い、オレンジ枠で表示していた。だがニアミス枠は必ずその人の「予定あり」ブロックと同じ時間帯に重なり、人名が二重に出る。誰の予定かは通常の「予定あり」ブロックで足りるため、`WeeklySlotFinder` を `all_available?` のみ呼ぶ形に戻した（別紙A 第5版）。
...
2. **重なる「予定あり」ブロックの視覚的衝突**：複数人の予定を1つのカレンダーに重ねる設計（§6.6）上、別々の人の予定が同じ時間帯に重なると、時刻だけに基づく絶対配置では完全に重なって読めなくなる（下になった予定が隠れる）バグが元々あった。区間彩色の貪欲法で列を割り当て、横に並べて見せる形に変更した（`MeetingsHelper#layout_busy`）。

**その後の追加①（時刻軸・終日行）**：週カレンダー左端に時刻軸列（正時ラベル＋本体の薄い横罫）を、最上部に固定高の「終日」行を足した。縦位置の読み取りやすさのための表示のみの変更で、判定ロジック・データモデルには影響しない（`MeetingsHelper#hour_ticks` / `#tick_top`、`_week_grid.html.erb`）。軸列のトラック幅は `2.75rem` 固定（中身の正時ラベルが `position:absolute` で幅を持たず、`auto` だと終日予定の無い週で 0px に潰れるため）。

**その後の追加②（空き帯を直接クリック）**：当初は空き帯の中に開始時刻チップ（`13:00〜` 等）を30分おきに並べていたが、時刻軸を入れたことで「緑＝空き・軸で何時か読める」が成立し、同じ見た目のピルが縦に並ぶ表示は冗長になった。**空き帯（`a.seg--free`）そのものをクリック対象にし、押した縦位置を5分丸めした時刻を①bの開始時刻の初期値にする**方式へ変更（`slot_picker_controller.js`）。`href` は帯の先頭時刻で組んであり、JS無効時・キーボードの Enter（`clientY` が 0 になり先頭にクランプ）でもフォールバックとして予約フォームが開く。狭い画面でチップが折り返して帯の高さ（＝所要時間ぶんのpx）を超え `overflow:hidden` で切れていた問題も同時に解消。**帯のどこをクリックしても「その時刻から所要時間ぶん全員空き」は `WeeklySlotFinder` の走査で保証済み**（帯末尾＝所要が収まる最後の開始でクランプ）。他メンバーの予定と重なる時刻はそもそも帯に含まれず、その予定は別途グレーの「予定あり」ブロックとして描かれる。
...
Rails.application.routes.draw do
  ...
  resources :users # 画面③ メンバー管理（admin 限定。UsersController 側で制御）
...
| メソッド | パス | 用途 |
|---|---|---|
| GET / POST | `/login` | ログイン（メール＋パスワード） |
| DELETE | `/logout` | ログアウト |
| GET | `/meetings` | 予約一覧（今後分・全件共有） |
...
| GET | `/meetings/:id/edit` | 予約の変更フォーム（日時・所要時間・参加者・ゲスト情報）。画面①と同じ週カレンダー＋参加者サイドバー付き（判断メモ D-9・D-14） |
| PATCH/PUT | `/meetings/:id` | 予約の変更（再判定つき）。過去日時・競合は 422 |
| DELETE | `/meetings/:id` | 予約のキャンセル（占有を連動削除） |
| GET | `/users` ほか一式 | メンバー管理（admin 限定） |

`calendar` は `_calendar_frame.html.erb`（内側で `_week_grid.html.erb`）を返す。**空き枠だけでなく、埋まっている区間も「誰の何の予定か」を付けて渡す。** 表示側で消すかどうかを判断できるようにするためである。
...
| GET/POST/PATCH/DELETE | `/tenant_rules` | テナント全体の営業時間・固定ブロック | 画面⑤／F-11〜F-13（実装済み。admin限定） |

**残る未実装は画面④のみ。** これが無くても主動線（組む・一覧・変更・キャンセル）は完結する。個人の `availability_rules`（`user_id` あり）は引き続き `db/seeds.rb` が唯一の入力経路。詳細は §9.4。
...
| 4 | 認証はメール＋パスワードのみ（多要素認証・監査ログなし） | 社内ツールで本番データを扱わない。認可は `admin` による設定画面の保護のみ。MFA・監査・管理者画面の完全分離は Phase 2（F-52）。判断メモ D-1・D-2 |
...
| 8 | 空き枠の優先順位付け・自動抽出を行わない | 画面①（週カレンダー）は「空き」を可視化するのみで、おすすめ順に並べ替える等の機械的な抽出は Phase 2（F-17）。現行運用が目視判断のため、一覧化と可視化だけで工数は削減される |
...
| 3 | ゲストの希望日時が全滅したとき、次の行動を示せない | **解消。** 週カレンダー（画面①）が選択メンバー全員の空き枠を週単位で一望できるため、希望日時が埋まっていても代替候補がその場で見える（§6.4・§6.6）。空き枠の自動抽出（優先度付けなど）は引き続き Phase 2 |
...
| ID | 機能 | 画面 | 現状 |
|---|---|---|---|
| F-11〜F-13（個人ルールのみ） | 個人ごとの営業時間・予約不可時間の設定 | ― | テナント全体は `/tenant_rules` で実装済み（判断メモ D-11・D-13）。個人ルール（`user_id` あり）は `db/seeds.rb` 投入のみで、変更にはコード編集が要る |
| F-29 | カレンダー連携画面 | 画面④ | 未実装（元々「設計時に定義しておらず実装中に追加した」計画のみの機能。§6.5） |

**F-01/F-02（メンバー管理）・F-24（予約一覧）は実装済み**（判断メモ D-3・D-4）。**F-25（変更）・F-26（キャンセル）も Phase 2 から前倒しで実装済み**（判断メモ D-5。参加者の増減も D-14 で対応）。**F-11〜F-13（テナント全体分）も画面⑤として実装済み**（判断メモ D-11・D-13）。残る未着手は画面④と、個人ごとの `availability_rules` の設定画面。

**現行動線への影響**：主動線（組む・一覧・変更・キャンセル）は画面④⑤が無くても完結する。**評価対象である「確認から予約までを一度の操作で完結させる」という中核価値は実証済みの状態にある。**
...
| 管理者画面の分離 | `admin` フラグと `require_admin` は Phase 1 で実装済み。Phase 2 は「管理者／CA」の権限差の確定と、それに応じた画面の細分化 | 小 |
| ゲストが直接枠を選ぶ | 公開画面・URLトークン・**仮押さえ状態**（`meetings.status`）・排他制御。**書き込み経路が2本になるため、SQLiteの前提が崩れる（§2.2）** | 大 |
| 予約の変更・キャンセル | **Phase 1 で実装済み**（`edit`/`update`/`destroy`。`source` と `meeting_id` があるため素直だった）。参加者の増減も実装済み（判断メモ D-14）。Phase 2 で追加するのは「ゲスト・参加者への通知」 | 小 |
