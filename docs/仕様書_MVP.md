...
| **管理者** | ○（設定画面も） | △ | `users.operator = true` かつ `users.admin = true` |
...
| | `operator` | `participant` | `admin` |
|---|---|---|---|
| 主催者でミーティングにも出る | true | true | 任意 |
| 主催者で調整専門 | true | false | 任意 |
| ミーティングだけ出る社内メンバー | false | true | false |
| 設定画面（メンバー管理）を扱う | true | 任意 | **true** |

`admin` は設定画面（画面③メンバー管理）の操作可否だけを分ける1段階のフラグ。「管理者」と「CA」で操作をどう分けるかは未ヒヤリング（仮定A-6）で、細分化と管理者画面の完全分離は Phase 2（判断メモ D-2）。
...
| 項目 | 内容 |
|---|---|
| 認証 | メールアドレス（`operator: true` で絞る）＋パスワード（`bcrypt` / `password_digest`） |
...
**`operator` を毎リクエスト確認する理由**：セッションのIDだけで引くと、後から `operator` を false にされたユーザーがログインしたままになる。毎回確認することで権限を落とした瞬間に操作不能になる。
...
**権限による出し分けは設定メニュー（画面③メンバー管理）のみ。** `admin = true` のときだけ設定リンクを表示する。それ以外の画面は `operator` であれば区別しない（管理者とCAの操作差は未ヒヤリング＝仮定A-6）。右上に現在の操作者と「ログアウト」を常時表示する。
...
`resources :users` のフルCRUD。`before_action :require_admin` で `admin = true` のメンバーだけが操作できる（§6.2）。氏名・メール・パスワード・`operator`/`participant`/`admin` を編集する。
...
| # | 画面 | 誰が | 入力 | 出力 | 優先度 |
|---|---|---|---|---|---|
| ⓪ | ログイン | 主催者 | メールアドレス＋パスワード（`operator = true` のみ） | セッションに `user_id` を保持 | Must（◎実装済み） |
...
| ③ | メンバー | **管理者** | 氏名／メール／パスワード／`operator`／`participant`／`admin` | メンバー一覧 | Must（◎実装済み・admin限定） |
