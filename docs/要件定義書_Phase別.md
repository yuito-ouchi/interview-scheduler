...
2026-08-31 初版 ／ 2026-09-02 改訂（F-16の実装内容・未着手Must・用語統一〔面接→ミーティング／候補者→ゲスト／面接官→参加者／採用担当者→主催者〕を反映） ／ 情報源：Day 2 ヒヤリング面談 ／ 関連文書：仕様書（MVP）、別紙A（実装詳細）
...
| F-02 | `operator` / `participant` フラグ設定 | 前提 | H-02「管理者とCAがあれば良い」 |
...
**F-16・F-20・F-21・F-22（空き判定・重複防止という中核）を優先し、以下は着手できていない。** 主動線（ミーティングを組む・予約確定）はこれらが無くても完結する。詳細と現状は仕様書§9.4を参照。

| ID | 機能 | 優先度 |
|---|---|---|
| F-01 / F-02 | メンバー登録・編集・フラグ設定 | Must |
| F-24 | 予約一覧（詳細のみ実装済み） | Must |
| F-11〜F-13 | 営業時間・予約不可時間・土日ブロックの設定画面 | Should |
| F-29 | カレンダー連携画面 | 実装中に追加 |
...
| 出力 | 選択メンバーの予定を1つに重ねた週カレンダー。空き（全員可）・**あと1人で空き**（1名だけ不可。実装段階の追加）・予定あり（誰の何の予定か）・予約不可時間・営業日外の5状態を表示 |
...
| 出力 | 以降 `meetings.created_by_id` に記録される |
