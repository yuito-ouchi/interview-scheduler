...
# メンバーは全員ログインでき、参加者候補にも並ぶ。区別は admin だけ（判断メモ D-15）。
op1 = User.create!(name: "採用 花子", email: "hanako@example.com", password: DEV_PASSWORD, admin: true)
op2 = User.create!(name: "調整 太郎", email: "taro@example.com",   password: DEV_PASSWORD)

iv1 = User.create!(name: "伊藤 一郎", email: "ito@example.com",    password: DEV_PASSWORD)
iv2 = User.create!(name: "佐藤 次郎", email: "sato@example.com",   password: DEV_PASSWORD)
iv3 = User.create!(name: "鈴木 三郎", email: "suzuki@example.com", password: DEV_PASSWORD)
iv4 = User.create!(name: "田中 四郎", email: "tanaka@example.com", password: DEV_PASSWORD)
...
# --- サマリ ---------------------------------------------------------
puts "seed 完了"
puts "  users: #{User.count}（うち admin: #{User.admins.count}）"
